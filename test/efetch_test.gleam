...
fn echo_decoder() -> decode.Decoder(Echo) {
  ...
  use headers <- decode.field("headers", decode.dict(decode.string, decode.string))
  use parsed_query_params <- decode.field("parsedQueryParams", decode.dict(decode.string, decode.string))
  case string.uppercase(method) {
    "POST" | "PUT" -> {
      use raw_body <- decode.field("rawBody", decode.optional(decode.string))
      decode.success(Echo(method:, protocol:, host:, path:, ip:, headers:, parsed_query_params:, raw_body:))
    }
    _ -> decode.success(Echo(method:, protocol:, host:, path:, ip:, headers:, parsed_query_params:, raw_body: None))
  }
}

pub fn post_test() {
  let assert Ok(res) = request.Request(
    method: http.Post,
    headers: [
      #("Abc", "def"),
      #("Ghi", "jkl"),
  ...
  assert content_type == Ok("application/text") || content_type == Ok("application/octet-stream")
